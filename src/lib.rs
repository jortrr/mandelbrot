...
pub fn run(config: Config) -> Result<(), Box<dyn Error>> {
    ...
    let gcd = num::integer::gcd(config.width, config.height); //Needed to compute the aspect ratio of the pixel plane
    println!("Pixel plane: size is {}x{} and aspect ratio is {}:{}",config.width, config.height, config.width / gcd, config.height / gcd);
    c.print();
    ...
    render_complex_plane_into_buffer(&mut buffer, &c, config.width, config.height, config.orbit_radius, config.max_iterations);
    println!();
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        window.update_with_buffer(&buffer, config.width, config.height).unwrap();
        ...
        for key in window.get_keys_pressed(minifb::KeyRepeat::No) {
            ...
            match key {
                Key::Up => {c.translate(0.0, -c.increment_y * translation_amount as f64); translate_and_render_complex_plane_buffer(&mut buffer, &c, config.width, config.height, translation_amount as i128, 0, config.orbit_radius, config.max_iterations)},
                Key::Down => {c.translate(0.0, c.increment_y  * translation_amount as f64); translate_and_render_complex_plane_buffer(&mut buffer, &c, config.width, config.height, -(translation_amount as i128), 0, config.orbit_radius, config.max_iterations)},
                Key::Left => {c.translate(-c.increment_x  * translation_amount as f64, 0.0); translate_and_render_complex_plane_buffer(&mut buffer, &c, config.width, config.height, 0, translation_amount as i128, config.orbit_radius, config.max_iterations);},
                Key::Right => {c.translate(c.increment_x  * translation_amount as f64, 0.0); translate_and_render_complex_plane_buffer(&mut buffer, &c, config.width, config.height, 0, -(translation_amount as i128), config.orbit_radius, config.max_iterations);},
                Key::R => c.reset(),
        ...
        if let Some((x, y)) = window.get_mouse_pos(MouseMode::Discard) {
            ...
            if mouse_down_now && !mouse_down {
                println!("({x}, {y})");
                let index = point_to_index(x as usize, y as usize, config.width);
                let pixel = buffer[index];
                let iterations = iterations_from_hsv_pixel(pixel, config.max_iterations);
...
fn render_complex_plane_into_buffer(buffer: &mut Vec<u32>, c: &ComplexPlane, width: usize, height: usize, orbit_radius: f64, max_iterations: u32) {
...
fn render_box_render_complex_plane_into_buffer(buffer: &mut Vec<u32>, c: &ComplexPlane, width: usize, height: usize, orbit_radius: f64, max_iterations: u32, render_min_x: usize, render_max_x: usize, render_min_y: usize, render_max_y: usize) {
            ...
            loop {
                ...
                *data+=1;
                ...
                for (i, pixel) in chunk.iter_mut().enumerate() {
                    let point = index_to_point(i + chunk_start, width);
...
fn translate_and_render_complex_plane_buffer(buffer: &mut Vec<u32>, c: &ComplexPlane, width: usize, height: usize, rows: i128, columns: i128, orbit_radius: f64, max_iterations: u32) {
...
fn index_to_point(index: usize, width: usize) -> (usize, usize) {
